...
function getUnit(exercise: string): string {
...
export default function NewGroupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!canSubmit) return
    setLoading(true)
    // TODO: supabase.from('groups').insert({ name, exercise_name, created_by, invite_code })
      ...
      <div className="flex-1 px-4 pb-10">
        <form onSubmit={handleSubmit} className="app-card p-5 space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {PRESETS.map(p => (
                ...
                >
                  {p.name}
                  <span className={`ml-1 text-xs ${exercise === p.name ? 'text-white/70' : 'text-gray-400'}`}>
                    ({p.unit})
                  </span>
                </button>
              ))}
          ...
          <div>
            ...
            {exercise && (
              <p className="text-xs text-app-blue font-bold mb-2">
                単位：{unit}
                {unit !== '回' && `（${exercise}は${unit}で計測）`}
              </p>
            )}
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={() => setInitial(v => String(Math.max(1, Number(v) - 1)))}
              ...
              <button
                type="button"
                onClick={() => setInitial(v => String(Math.min(9999, Number(v) + 1)))}
                className="app-btn w-12 h-12 bg-app-navy text-white rounded-2xl text-xl font-bold active:scale-90"
              >
                ＋
              </button>
            </div>
            <p className="text-gray-400 text-xs font-bold mt-1.5 text-center">
              毎日1{unit}ずつ自動で増えます
            </p>
          </div>
