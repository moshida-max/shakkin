...
export default function JoinGroupPage() {
  ...
  const [found, setFound]       = useState<typeof MOCK_GROUPS[0] | null>(null)
  ...
  const handleJoin = async () => {
    if (!found) return
    setJoining(true)
    // TODO: supabase.from('memberships').insert(...)
    ...
    <div className="min-h-screen bg-app-teal flex flex-col">
      ...
      <div className="flex-1 px-4 space-y-4 pb-10">
        ...
        <div className="app-card p-5">
          <label className="block font-bold text-app-navy text-sm mb-3">
            招待コード（6文字）
          </label>
          <div className="flex gap-2 mb-3">
            ...
            <button
              ...
              className={`rounded-2xl px-5 py-3 font-bold text-sm transition-all active:scale-95 ${
                code.length === 6 && !searching
                  ? 'bg-app-navy text-white'
                  : 'bg-gray-100 text-gray-300 cursor-not-allowed'
              }`}
              style={code.length === 6 ? { boxShadow: '0 3px 10px rgba(0,0,0,0.15)' } : {}}>
              {searching ? '...' : '検索'}
            </button>
          </div>
          {error && (
            <div className="flex items-center gap-2 text-app-red text-sm font-bold">
              <span>😢</span> {error}
            </div>
          )}
          {!found && !error && (
            <p className="text-gray-400 text-xs font-bold text-center">
              📩 メンバーから招待コードを受け取って入力しよう
            </p>
          )}
        </div>
        ...
        {found && (
          <div className="app-card overflow-hidden">
            <div className="bg-app-yellow px-5 py-4 flex items-center justify-between">
              <div>
              ...
              <StickFigure pose="celebrate" size={52} color="#1A1A2E" />
            </div>
            <div className="p-5 space-y-4">
              <div>
                <label className="block font-bold text-app-navy text-sm mb-2">
                  初期回数 <span className="text-gray-400 text-xs font-bold">（1日目のノルマ）</span>
                </label>
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => setInitial(v => Math.max(1, v - 1))}
                    className="app-btn w-12 h-12 bg-gray-100 text-gray-600 rounded-2xl text-xl font-bold active:scale-90">
                    −
                  </button>
                  ...
                  <button type="button" onClick={() => setInitial(v => Math.min(999, v + 1))}
                    className="app-btn w-12 h-12 bg-app-navy text-white rounded-2xl text-xl font-bold active:scale-90">
                    ＋
                  </button>
                </div>
              </div>
