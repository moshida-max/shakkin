...
export default function GroupSettingsPage() {
  ...
  async function loadData() {
    ...
    const { data: m } = await supabase.from('memberships').select('*')
      .eq('group_id', groupId).eq('user_id', uid).single()
    if (g) { setGroup(g); setIsCreator(g.created_by === uid) }
    if (m) { setMembership(m); setInitialReps(m.initial_reps) }
    setLoading(false)
  }

  const handleSaveReps = async () => {
    if (!membership) return
    await supabase.from('memberships').update({ initial_reps: initialReps }).eq('id', membership.id)
    setRepsSaved(true)
    ...
    <div className="min-h-screen bg-app-pink flex flex-col">
      ...
      <div className="flex-1 px-4 pb-16 space-y-4">
        ...
        <div className="app-card p-5">
          ...
          <div className="text-gray-400 text-xs font-bold mb-5">1日目のノルマ。毎日 1{unit} ずつ自動で増えます。</div>
          <div className="flex items-center gap-3 mb-4">
            <button onClick={() => setInitialReps(v => Math.max(1, v - 1))}
            ...
            <div className="flex-1 flex items-center gap-2 justify-center">
              <input type="number" value={initialReps}
            ...
            <button onClick={() => setInitialReps(v => Math.min(9999, v + 1))}
          ...
          <button onClick={handleSaveReps}
            className={`w-full rounded-2xl py-4 font-bold text-base transition-all active:scale-95 ${repsSaved ? 'bg-app-green text-white' : 'bg-app-navy text-white'}`}
        ...
        <div className="app-card p-5">
          ...
          <div className="text-gray-400 text-xs font-bold mb-4">退会するとこのグループでの記録が削除されます。</div>
          <button onClick={() => setShowLeave(true)}
            className="w-full rounded-2xl py-4 font-bold text-base text-app-red bg-red-50 active:scale-95 transition-all">
            退会する
          </button>
        </div>
        ...
        {isCreator && (
          <div className="app-card p-5">
            ...
            <div className="text-gray-400 text-xs font-bold mb-4">全メンバーの記録が完全に削除されます。取り消せません。</div>
      ...
      {showLeave && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end" onClick={() => setShowLeave(false)}>
          <div className="w-full max-w-[390px] mx-auto bg-white rounded-t-3xl p-6 pb-10" onClick={e => e.stopPropagation()}>
            ...
            <div className="text-gray-400 text-sm mb-6">「{group.name}」の記録が削除されます。</div>
            <button onClick={handleLeave} className="w-full rounded-2xl py-4 bg-app-red text-white font-bold text-base active:scale-95 transition-all mb-2">退会する</button>
            <button onClick={() => setShowLeave(false)} className="w-full py-3 font-bold text-gray-400 text-sm">キャンセル</button>
          </div>
        </div>
      )}
      ...
      {showDelete && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end" onClick={() => setShowDelete(false)}>
          <div className="w-full max-w-[390px] mx-auto bg-white rounded-t-3xl p-6 pb-10" onClick={e => e.stopPropagation()}>
            ...
            <div className="text-gray-400 text-sm mb-2">「{group.name}」の全データが削除されます。</div>
            <div className="bg-red-50 rounded-2xl px-4 py-3 mb-6">
              <div className="text-app-red font-bold text-xs">⚠️ この操作は取り消せません</div>
            </div>
            <button onClick={handleDelete} className="w-full rounded-2xl py-4 bg-app-red text-white font-bold text-base active:scale-95 transition-all mb-2"
              style={{ boxShadow: '0 4px 16px rgba(255,92,92,0.3)' }}>完全に削除する</button>
            <button onClick={() => setShowDelete(false)} className="w-full py-3 font-bold text-gray-400 text-sm">キャンセル</button>
          </div>
        </div>
      )}
    </div>
